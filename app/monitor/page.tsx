...
export default function MonitorPage() {
  ...
  // Auto-trigger Watcher analysis when meds load (once per session)
  useEffect(() => {
    if (watchedMeds.length >= 2 && !analysing && typeof sessionStorage !== 'undefined') {
      const key = `phlox_watcher_done_${user?.id}`
