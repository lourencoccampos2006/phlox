  ...
  if (index >= cards.length) {
    return (
      <div style={{ textAlign: 'center', padding: '60px 40px' }}>
        <div style={{ fontFamily: 'var(--font-serif)', fontSize: 32, color: 'var(--ink)', marginBottom: 12 }}>
          Sessão concluída
        </div>
        <p style={{ fontSize: 16, color: 'var(--ink-3)', marginBottom: 8 }}>
          {known.size} de {cards.length} cartões marcados como conhecidos
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 24 }}>
          <button onClick={() => { setIndex(0); setFlipped(false); setKnown(new Set()) }}
            style={{ background: 'var(--green)', color: 'white', border: 'none', borderRadius: 4, padding: '10px 24px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
            Repetir
          </button>
          <button onClick={onBack}
            style={{ background: 'var(--bg-3)', color: 'var(--ink-2)', border: 'none', borderRadius: 4, padding: '10px 24px', fontSize: 14, cursor: 'pointer' }}>
            Voltar
          </button>
        </div>
      </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div style={{ fontSize: 13, color: 'var(--ink-3)', fontFamily: 'var(--font-mono)' }}>
          {drugClass} · {index + 1} / {cards.length}
        </div>
        <div style={{ fontSize: 13, color: 'var(--green-2)', fontFamily: 'var(--font-mono)' }}>
          {known.size} conhecidos
        </div>
      </div>
      ...
      >
        ...
        {!flipped ? (
          <p style={{ fontFamily: 'var(--font-serif)', fontSize: 22, color: 'var(--ink)', lineHeight: 1.5, maxWidth: 500 }}>
            {card.front}
          </p>
        ) : (
          <p style={{ fontSize: 15, color: 'var(--ink-2)', lineHeight: 1.8, maxWidth: 560 }}>
            {card.back}
          </p>
        )}
      </div>
...
function QuizMode({ drugClass, questions, onBack }: { drugClass: string; questions: QuizQuestion[]; onBack: () => void }) {
  ...
  if (index >= questions.length) {
    ...
    return (
      <div style={{ textAlign: 'center', padding: '60px 40px' }}>
        <div style={{ fontFamily: 'var(--font-serif)', fontSize: 32, color: 'var(--ink)', marginBottom: 8 }}>
          Quiz concluído
        </div>
        <div style={{ fontSize: 48, fontWeight: 700, color: pct >= 70 ? 'var(--green)' : pct >= 50 ? '#dd6b20' : '#c53030', margin: '16px 0' }}>
          {pct}%
        </div>
        <p style={{ fontSize: 16, color: 'var(--ink-3)', marginBottom: 24 }}>
          {score} respostas correctas em {questions.length} perguntas
        </p>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--ink-3)' }}>
          {drugClass} · {index + 1} / {questions.length}
        </div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--green-2)' }}>
          {score} correctas
        </div>
      </div>
      ...
      <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 6, padding: '28px 28px 20px', marginBottom: 16 }}>
        <p style={{ fontFamily: 'var(--font-serif)', fontSize: 19, color: 'var(--ink)', lineHeight: 1.55, marginBottom: 24, margin: '0 0 24px' }}>
          {q.question}
        </p>
...
export default function StudyPage() {
  ...
  const startFlashcards = async (dc: string) => {
    ...
    setSelectedClass(dc)
    try {
      const cards = await generateFlashcards(dc)
      setFlashcards(cards)
      setMode('flashcards')
    } catch {
      setError('Erro ao gerar flashcards. Tenta novamente.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const startQuiz = async (dc: string) => {
    ...
    setSelectedClass(dc)
    try {
      const questions = await generateQuiz(dc)
      setQuiz(questions)
      setMode('quiz')
    } catch {
      setError('Erro ao gerar quiz. Tenta novamente.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fafaf9', fontFamily: 'var(--font-sans)' }}>
      ...
      <header style={{ background: 'white', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 40px', height: 56, display: 'flex', alignItems: 'center', gap: 20 }}>
          ...
          <span style={{ fontSize: 14, color: 'var(--ink-3)' }}>
            {mode === 'home' ? 'Plataforma de Estudo' : mode === 'flashcards' ? `Flashcards · ${selectedClass}` : `Quiz · ${selectedClass}`}
          </span>
          {mode !== 'home' && (
            <button
              onClick={() => { setMode('home'); setError('') }}
              style={{ marginLeft: 'auto', background: 'none', border: 'none', fontSize: 13, color: 'var(--ink-3)', cursor: 'pointer', fontFamily: 'var(--font-sans)' }}
            >
              ← Voltar
            </button>
          )}
          {mode === 'home' && (
            <nav style={{ marginLeft: 'auto', display: 'flex', gap: 28 }}>
              <Link href="/interactions" style={{ fontSize: 13, color: 'var(--ink-3)', textDecoration: 'none', fontWeight: 500 }}>Interações</Link>
              <Link href="/drugs" style={{ fontSize: 13, color: 'var(--ink-3)', textDecoration: 'none', fontWeight: 500 }}>Medicamentos</Link>
            </nav>
          )}
        </div>
      </header>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 40px 80px' }}>
        ...
        {/* Error */}
        {error && (
          <div style={{ background: 'white', border: '1px solid #feb2b2', borderLeft: '4px solid #c53030', borderRadius: 4, padding: '14px 18px', marginBottom: 24, fontSize: 14, color: '#742a2a' }}>
            {error}
          </div>
        )}
        ...
        {loading && (
          <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 6, padding: '48px', textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--green)', letterSpacing: '0.15em', marginBottom: 12 }}>
              A GERAR CONTEÚDO
            </div>
        ...
        {!loading && mode === 'home' && (
          <div>
            <div style={{ marginBottom: 40 }}>
              <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 32, color: 'var(--ink)', marginBottom: 8, letterSpacing: '-0.01em' }}>
                Plataforma de Estudo
              </h1>
              <p style={{ fontSize: 15, color: 'var(--ink-4)', lineHeight: 1.6 }}>
                Flashcards e quizzes gerados por IA para estudantes de farmácia, medicina e enfermagem.
                Escolhe uma classe farmacológica para começar.
              </p>
            </div>
