...
export default function WardPage() {
  ...
  const sendMessage = async (type: WardMessage['type'], content: string, meta?: any) => {
    ...
    // Use ref first (avoids stale closure issue), fall back to state
    const channelId = channelIdRef.current || channels[selectedPatient.id]
    if (!channelId) {
      setSendError('Canal ainda a carregar. Aguarda um momento e tenta novamente.')
      return
    }
    setSendError('')
    setSending(true)
    const { data: msg, error: insertErr } = await supabase.from('channel_messages').insert({
    ...
    if (insertErr) {
      console.error('Ward insert error:', insertErr)
      setSendError(insertErr.message || 'Erro ao guardar mensagem.')
    } else if (msg) {
            ...
            <>
              ...
              {tab === 'handover' ? (
                ...
                <>
                  ...
                  {/* Compose */}
                  {sendError && (
                    <div style={{ padding: '8px 16px', background: '#fee2e2', borderTop: '1px solid #fca5a5', fontSize: 12, color: '#991b1b', fontFamily: 'var(--font-mono)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span>{sendError}</span>
                      <button onClick={() => setSendError('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#991b1b', fontSize: 16 }}>×</button>
                    </div>
                  )}
