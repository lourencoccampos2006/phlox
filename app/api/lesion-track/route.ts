...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Detalhe técnico (bucket em falta vs políticas de RLS em falta) só para o
    // dev, na consola do servidor — o cliente vê uma mensagem calma.
    console.error('[phlox:lesion-track-upload]', String(e?.message || e))
    return NextResponse.json({ error: 'Não foi possível guardar a foto agora. Tenta de novo.' }, { status: 500 })
  }
