...
function ActiveChallenge({ ch, onComplete, onAbandon }: {
  ch: Challenge
  onComplete: (xp: number, correct: boolean, time: number) => void
  onAbandon: () => void
}) {
  ...
  if (!ch.case_data?.options?.length || !ch.case_data?.presentation) {
    return (
      <div style={{ padding:24, background:'white', border:'1px solid var(--border)', borderRadius:10, textAlign:'center' }}>
        ...
        <button onClick={onAbandon} style={{ padding:'8px 16px', background:'var(--ink)', color:'white', border:'none', borderRadius:7, cursor:'pointer', fontSize:13, fontWeight:700, fontFamily:'var(--font-sans)' }}>
          Voltar
        </button>
      </div>
    )
  ...
  useEffect(() => {
    timerRef.current = setInterval(() => {
      setRemaining(r => {
        if (r <= 1) { handleSubmit(selected); return 0 }
        return r - 1
      })
      setTimeSpent(t => t + 1)
    }, 1000)
  ...
  const handleSubmit = (idx: number | null) => {
    ...
    const finalXp = correct ? baseXp + bonus : 0
    setTimeout(() => onComplete(finalXp, !!correct, timeSpent), 2800)
  }

  return (
    <div>
      <button onClick={onAbandon}
        style={{ background:'none', border:'none', fontSize:13, color:'var(--ink-4)', cursor:'pointer', fontFamily:'var(--font-sans)', padding:0, marginBottom:16, display:'flex', alignItems:'center', gap:6 }}>
        ← Abandonar
      </button>
        ...
        <div style={{ background:d.bg, borderBottom:`1px solid ${d.color}30`, padding:'14px 18px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
            ...
            <span style={{ marginLeft:'auto', fontSize:10, color:'var(--ink-4)', fontFamily:'var(--font-mono)' }}>
              {DOMAINS.find(x => x.id === ch.domain)?.icon} {DOMAINS.find(x => x.id === ch.domain)?.label || ch.domain}
            </span>
          </div>
          {!submitted && <TimerBar total={totalTime} remaining={remaining} />}
        </div>
        <div style={{ padding:'20px 18px' }}>
          <p style={{ fontSize:15, color:'var(--ink)', lineHeight:1.8, marginBottom:16, fontFamily:'var(--font-serif)' }}>
            {ch.case_data.presentation}
          </p>
...
export default function ArenaPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // Load challenges
      const { data: ch } = await supabase.from('arena_challenges')
      ...
      // Load MY attempts
      const { data: myAttempts } = await supabase.from('arena_attempts')
        .select('challenge_id, score, time_seconds, domain, completed_at')
        .eq('user_id', user.id)
      ...
      // Load leaderboard (all users - public policy)
      const { data: lb } = await supabase.from('arena_leaderboard')
        .select('*').order('total_xp', { ascending:false }).limit(50)

      const lbData = (lb || []).map((row: any, i: number) => ({
        user_id: row.user_id,
      ...
      setMyStats({
        total_xp: totalXp,
        total_attempts: ma.length,
        correct,
        streak: 0,
        league: getLeague(totalXp).id,
        rank: myRank || 0,
        by_domain: byDomain,
      })
    } catch (e) {
  ...
  const generateChallenge = async () => {
    ...
    try {
      ...
      const domain = filterDomain === 'all'
        ? DOMAINS[Math.floor(Math.random() * (DOMAINS.length - 1)) + 1].id
        : filterDomain
      const diff = filterDiff === 'all'
      ...
      alert(e.message || 'Erro ao gerar caso')
    }
    setGenerating(false)
  }
  ...
  const progress = myStats && nextLeague
    ? Math.round(((myStats.total_xp - myLeague.min) / (nextLeague.min - myLeague.min)) * 100)
    : 100
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)' }}>
      ...
      <div className="page-container page-body" style={{ maxWidth:480, margin:'0 auto' }}>
        <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:12, padding:'56px 32px', textAlign:'center' }}>
          <div className="eyebrow" style={{ justifyContent:'center', color:'#7c3aed', marginBottom:16 }}>Phlox Arena</div>
          <div style={{ fontFamily:'var(--font-serif)', fontSize:28, color:'var(--ink)', marginBottom:12 }}>Sistema de ligas clínicas</div>
          <p style={{ fontSize:14, color:'var(--ink-3)', lineHeight:1.7, marginBottom:28 }}>
            Bronze → Diamante. Casos de 8 domínios da saúde. XP real. Leaderboard global. Disponível no plano Student.
          </p>
          <Link href="/pricing" style={{ display:'inline-block', background:'#7c3aed', color:'white', textDecoration:'none', padding:'13px 32px', borderRadius:8, fontSize:14, fontWeight:700 }}>
            Ver plano Student →
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', fontFamily:'var(--font-sans)' }}>
        ...
        <div className="page-container" style={{ paddingTop:20, paddingBottom:0 }}>
          {myStats && (
            <div style={{ display:'flex', alignItems:'center', gap:16, marginBottom:16, flexWrap:'wrap' }}>
              <div style={{ fontSize:32 }}>{myLeague.icon}</div>
      ...
      <div className="page-container page-body">
        ...
        {active && (
          <ActiveChallenge ch={active} onComplete={handleComplete} onAbandon={() => setActive(null)} />
        )}
          ...
          <div>
            ...
            {loading ? (
              ...
              <div style={{ textAlign:'center', padding:'40px 0', color:'var(--ink-4)', fontSize:14 }}>
                Clica em "Gerar novo caso" para começar.
              </div>
            ) : filtered.map(ch => {
              const d = DIFF[ch.difficulty]
              ...
              return (
                <div key={ch.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', background:'white', border:'1px solid var(--border)', borderRadius:9, marginBottom:6, opacity:done?0.55:1, cursor:done?'default':'pointer', transition:'all 0.15s' }}
                  ...
                  <div style={{ fontSize:20, flexShrink:0 }}>{dom?.icon||'🏥'}</div>
                  <div style={{ flex:1, minWidth:0 }}>
                  ...
                  {done
                    ? <span style={{ fontSize:11, color:'#0d6e42', fontFamily:'var(--font-mono)', fontWeight:700, flexShrink:0 }}>✓ Feito</span>
        ...
        {!active && tab === 'league' && (
          <div>
            {/* My league card */}
            {myStats && (
              <div style={{ background:myLeague.bg, border:`2px solid ${myLeague.border}`, borderRadius:12, padding:'18px 20px', marginBottom:14, display:'flex', alignItems:'center', gap:14 }}>
                <div style={{ fontSize:44 }}>{myLeague.icon}</div>
                <div style={{ flex:1 }}>
                  ...
                  {nextLeague && (
                    <>
                      ...
                      <div style={{ fontSize:10, color:myLeague.color, opacity:0.6, fontFamily:'var(--font-mono)' }}>
                        {nextLeague.min - myStats.total_xp} XP para {LEAGUES.find(l=>l.id===nextLeague.id)?.icon} {nextLeague.label}
                      </div>
                    </>
                  )}
                </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(130px,1fr))', gap:8, marginBottom:14 }}>
              {LEAGUES.map(l => (
                <div key={l.id} style={{ background:l.bg, border:`1px solid ${l.border}`, borderRadius:8, padding:'10px 12px', opacity:myStats&&myStats.total_xp>=l.min?1:0.35, transition:'opacity 0.2s' }}>
                  <div style={{ fontSize:22, marginBottom:4 }}>{l.icon}</div>
                  <div style={{ fontSize:13, fontWeight:700, color:l.color }}>{l.label}</div>
                  <div style={{ fontSize:10, color:l.color, opacity:0.7, fontFamily:'var(--font-mono)' }}>{l.min}+ XP</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:10, overflow:'hidden' }}>
              <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--border)', background:'var(--bg-2)' }}>
                <div style={{ fontSize:10, fontFamily:'var(--font-mono)', fontWeight:700, color:'var(--ink)', textTransform:'uppercase', letterSpacing:'0.1em' }}>
                  Ranking Global · Top {leaderboard.length}
                </div>
              </div>
              {leaderboard.length === 0 ? (
                <div style={{ padding:'32px 16px', textAlign:'center', color:'var(--ink-4)', fontSize:13 }}>
                  Ainda sem dados suficientes. Joga casos para aparecer no ranking!
                </div>
              ) : leaderboard.map((p, i) => {
                ...
                return (
                  <div key={p.user_id} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 16px', borderBottom:i<leaderboard.length-1?'1px solid var(--bg-3)':'none', background:isMe?'#faf5ff':'white' }}>
                    <div style={{ width:26, height:26, borderRadius:'50%', background:i<3?['#ffd70020','#c0c0c020','#cd7f3220'][i]:'var(--bg-2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontFamily:'var(--font-mono)', fontWeight:700, color:'var(--ink-3)', flexShrink:0 }}>
                      {i<3?['🥇','🥈','🥉'][i]:`#${i+1}`}
                    </div>
        ...
        {!active && tab === 'stats' && myStats && (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(130px,1fr))', gap:10 }}>
              {[
                { label:'XP Total',     value:myStats.total_xp,   color:'#7c3aed', icon:'⚡' },
                { label:'Correctas',    value:`${myStats.correct}/${myStats.total_attempts}`, color:'#0d6e42', icon:'✓' },
                { label:'Taxa',         value:myStats.total_attempts>0?`${Math.round((myStats.correct/myStats.total_attempts)*100)}%`:'—', color:'#1d4ed8', icon:'📊' },
                { label:'Rank global',  value:myStats.rank>0?`#${myStats.rank}`:'—', color:'#d97706', icon:'🏆' },
              ].map(s => (
            ...
            {Object.keys(myStats.by_domain).length > 0 && (
              <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:10, padding:18 }}>
                ...
                {Object.entries(myStats.by_domain)
                    ...
                    return (
                      <div key={domain} style={{ marginBottom:10 }}>
                        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                          <span style={{ fontSize:12, color:'var(--ink)' }}>{dom?.icon||'🏥'} {dom?.label||domain}</span>
            ...
            <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:10, padding:18 }}>
              ...
              <div style={{ display:'flex', gap:0, borderRadius:8, overflow:'hidden', border:'1px solid var(--border)' }}>
                {LEAGUES.map((l, i) => {
                  ...
                  return (
                    <div key={l.id} style={{ flex:1, background:unlocked?l.bg:'var(--bg-3)', padding:'10px 4px', textAlign:'center', borderRight:i<LEAGUES.length-1?'1px solid rgba(255,255,255,0.3)':'none' }}>
                      <div style={{ fontSize:18, marginBottom:2, opacity:unlocked?1:0.3 }}>{l.icon}</div>
