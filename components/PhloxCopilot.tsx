...
export default function PhloxCopilot() {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/copilot-chat', {
        ...
        body: JSON.stringify({ message: q, path: pathname, selection, history: newMsgs.slice(-6) }),
      })
  ...
  return (
    <>
      {/* Botão flutuante */}
      {!open && (
        <button onClick={() => { const s = window.getSelection()?.toString() || ''; setSelection(s.slice(0, 1500)); setOpen(true) }}
          aria-label="Abrir Phlox Copilot"
          style={{
            position: 'fixed', bottom: 20, right: 20, zIndex: 9000,
            width: 52, height: 52, borderRadius: '50%', border: '1px solid #23262d',
            background: '#16181d', color: 'white', cursor: 'pointer', boxShadow: '0 6px 24px rgba(22,24,29,0.18)',
          ...
          <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
            {msgs.length === 0 && (
              <div style={{ color: '#6b7280', fontSize: 13, lineHeight: 1.6 }}>
                Pergunta-me qualquer coisa sobre o que estás a ver. Sei em que página estás{selection ? ' e o que selecionaste' : ''}.
