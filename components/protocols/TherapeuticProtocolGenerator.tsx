...
// Gerador de protocolo terapêutico por IA (era a página /protocol) — diferente dos
// protocolos institucionais (SOPs escritos e reutilizáveis): aqui a IA gera um plano
// de tratamento ad-hoc para o contexto de UM doente, com fármacos/doses/alvos/
// monitorização baseado em guidelines. Fundido em /protocolos como segundo separador.
...
export default function TherapeuticProtocolGenerator() {
  ...
  const generate = async (q?: string) => {
    ...
    try {
      ...
      const res = await fetch('/api/protocol', { method: 'POST', headers, body: JSON.stringify({ patient: term }) })
