...
// Escalas & Turnos — era a página própria /schedule, agora um separador dentro
// de /equipa. Mantém os seus próprios sub-separadores (Estado de serviço,
// Escalas, Tarefas, Configurar) tal como já existiam.
...
export default function EscalasEquipa({ initialSubTab }: { initialSubTab?: SubTab } = {}) {
  const { user, supabase } = useAuth()
  const scope = useOrgScope()

  const [tab, setTab] = useState<SubTab>(initialSubTab || 'team')
              ...
              {selectedMember && (
                <div className="team-detail" style={{ width: 250, flexShrink: 0, background: 'white', border: '1px solid var(--border)', borderRadius: 12, padding: 18, position: 'sticky', top: 20 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                    ...
                    <button aria-label="Fechar" onClick={() => setSelectedId(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, color: 'var(--ink-5)', padding: '0 4px', lineHeight: 1 }}>×</button>
                  </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 400, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
          ...
          <div style={{ background: 'white', borderRadius: '16px 16px 0 0', width: '100%', maxWidth: 520, padding: '22px 22px 40px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              ...
              <button aria-label="Fechar" onClick={() => setShowMemberForm(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 22, color: 'var(--ink-4)', lineHeight: 1 }}>×</button>
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 400, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
          ...
          <div style={{ background: 'white', borderRadius: '16px 16px 0 0', width: '100%', maxWidth: 520, padding: '22px 22px 40px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              ...
              <button aria-label="Fechar" onClick={() => setShowShiftForm(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 22, color: 'var(--ink-4)', lineHeight: 1 }}>×</button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
              <div>
                <Lbl>Funcionário *</Lbl>
