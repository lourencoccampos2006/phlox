// lib/interactionsEngine.ts
// Motor de verificação de interações medicamentosas — RxNorm/NIH real (dados
// determinísticos) + análise por IA verificada em 2 passagens (aiJSONVerified).
// Extraído de app/api/interactions/route.ts (Ronda 9) para ser reutilizado pelo
// Phlox Copilot (lib/copilotTools.ts) — o Copilot passa a usar a MESMA
// verificação rigorosa da ferramenta dedicada, em vez de adivinhar.
...
export interface InteractionResult {
  ...
  patient_info: string
  references: string[]
  source: 'ai_enhanced' | 'rxnorm' | 'none_found'
  drugs: string[]
  cached?: boolean
}

const cache = new Map<string, { result: InteractionResult; timestamp: number }>()
...
export function sanitizeDrugs(input: string[]): string[] {
  return input.map(sanitize).filter(Boolean)
}
...
/** Analisa interações entre 2-10 fármacos (RxNorm real + IA verificada), com cache de 24h. */
export async function checkInteractions(drugsRaw: string[]): Promise<InteractionResult> {
  const drugs = sanitizeDrugs(drugsRaw)
  if (drugs.length < 2 || drugs.length > 10) {
    throw Object.assign(new Error('Entre 2 e 10 substâncias'), { code: 'invalid_count' })
  }

  const key = [...drugs].sort().join('|').toLowerCase()
  const cached = cache.get(key)
  if (cached && Date.now() - cached.timestamp < CACHE_TTL) return { ...cached.result, cached: true }
  ...
  if (aiResult.status === 'fulfilled' && aiResult.value) {
    ...
    const severity = mapSev(worst.severity) as InteractionResult['severity']
    result = {
      severity,
