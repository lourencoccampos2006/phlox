...
class PhloxLogo extends HTMLElement {
  ...
  async _boot() {
    ...
    /* ─── ALTERAÇÃO AO PACOTE ORIGINAL (2026-08-25) ──────────────────────
     * Dois atributos novos, pedidos para o rodapé do Phlox:
     *
     *   sempre-a-rodar  ignora o prefers-reduced-motion e roda na mesma.
     *   sem-interacao   não regista os eventos de ponteiro: tocar não faz nada.
     *
     * Sem eles o comportamento é exatamente o do pacote original.
     * ──────────────────────────────────────────────────────────────────── */
    const reduce = matchMedia('(prefers-reduced-motion: reduce)');
    const sempreARodar = this.hasAttribute('sempre-a-rodar');
    const frozen = () => this.hasAttribute('static') || (reduce.matches && !sempreARodar);
    let visible = true, drag = null, manual = 0;
    this._io = new IntersectionObserver((e) => { visible = e[0].isIntersecting; }, { threshold: 0.05 });
    this._io.observe(this);

    if (!this.hasAttribute('sem-interacao')) {
      ...
      // Sem interação o cursor de "agarrar" seria uma promessa falsa.
